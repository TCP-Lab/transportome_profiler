...
acyclic graph, more specifically as a tree \todo{We should think about this some more... see comment in latex code}. In this tree, each node represents a
% dovremmo forse riconsiderare la struttura ad albero a favore della più libera
% (ma anche più complessa) struttura del DAG? in fondo la  multiple class
% inheritance dei DAG sembra essere fatta apposta per rappresentare alcuni gene
% set come "canali del calcio voltaggio dipendenti" che dovrebbero essere figli
% di "canali ionici voltaggio dipendenti" e "canali ionici permeabili al calcio".
% Questo renderebbe la struttura davvero analoga a quella di un'ontologia,
% ma soprattutto avrebbe il vantaggio di eliminare gran parte della ridondanza
% alla radice e in modo pulito...in realtà l'algoritmo di generazione dei gene
% set potrebbe rimanere tranquillamente lo stesso, ma si dovrebbero poi definire
% i rapporti figlio-genitore in un secondo momento sulla base degli elementi in
% ciascun gene set. Il vantaggio più grande sarebbe un aumento di leggibilità
% della struttura. Il problema più grande sarebbe forse la rappresentazione
% del DAG a lato della heatmap.
...
alike the gene lists present in the \gls{go}. % not so alike...
...
This iterative process generates a very large number of gene sets \todo{how many?}.
...
% MAYBE THIS IS WRONG? Generally close to 0? Yeah, it was wrong... but now it's right
