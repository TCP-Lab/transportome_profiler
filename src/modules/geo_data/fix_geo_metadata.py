...
OPERATIONS: dict[str, Callable] = {
    "GSE107422": chain(
        apply_at("sample_id", chain(take_field(" ", -1), replace("-", "."))),
        apply_at("var_0", follow_map({"primary colorectal cancer": "case"})),
        set_to("type", "COAD")
    ),
    "GSE121842": chain(
        apply_at("sample_id", take_field(" ", 0)),
        apply_at(
            "var_0",
            chain(
                take_field(" ", 1),
                follow_map({"cancer": "case", "pericarcinomatous": "control"})
            )
        ),
        set_to("type", "COAD")
    ),
    "GSE159857": chain(
        ...
        apply_at("var_0", follow_regmap({"^Tumor": "case", "^Normal": "control"}))
    ),
    "GSE201284": chain(
        apply_at("sample_id", take_field(", ", 1)),
        set_to("type", "PRAD"),
        apply_at("var_0", follow_map({"buffy coat": "buffy_coat", "prostate tissue": "case"}))
    )
}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--id", type=str, help="Accession ID. If not specified, uses the name of the metadata file.")

    args = parser.parse_args()

    main(metadata=args.metadata, accession=args.id if args.id else stem_of(args.metadata))
