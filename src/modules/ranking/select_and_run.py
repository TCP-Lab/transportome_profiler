...
def run_wrapper(
    keyvalue, input_matrix_path, input_metadata_path, output_dir, delimiter, method
):
    key, value = keyvalue
    set_meta = partial(
        replace,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "output_dir", type=Path, help="Output directory to save files to"
    )
    parser.add_argument("--delimiter", default=",", help="Delimiter for the input")
    parser.add_argument(
        "--cpus",
        type=int,
        help="Number of CPUS to use. If unspecified, runs with one thread per available core.",
    )
    parser.add_argument(
        "--method",
        type=str,
